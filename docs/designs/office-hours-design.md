...
中小嵌入式团队（如 P0133 产品线）的日常：构建靠裸编译服务器 + 手敲命令，刷机靠 SecureCRT + VBS 脚本，测试用例管在 Excel，测试报告手写 Word，发布靠手动拷贝。全流程割裂、无记录、无追溯，每个版本重复一遍。现有开源工具（LAVA、LabGrid、pytest-embedded、Renode）全部假设「串口在服务器上、一切可自动化」，不匹配中小团队「串口插在个人电脑 + 人机混合测试」的现实。
...
- 单台编译服务器（<server-lan-ip>），Go + Vue3 + SQLite，单二进制自托管
...
1. 团队日常使用：P0133 的构建-部署-测试-发布全流程在 EmbedFlow 上跑通，SecureCRT + Excel + 手写报告退役
...
1. **M1 会话层原型**（估时 4-6 周，决策 15A；含最小客户端工作量）：
   ...
   - **验收：用真实 VBS 脚本（<vbs-script>.vbs）翻译成 expect 规则（经编辑 UI 录入），全链路实测通过**（含透传延迟在真实设备 uboot 阶段的时序验证）
...
- 真实 VBS 刷机脚本（<local-path>\<vbs-script>.vbs）—— expect 引擎的验收基准样本
- Word 报告模板（<local-path>模板.docx）—— M4 报告导出的模板基准
